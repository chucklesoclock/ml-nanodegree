...
def set_all_defaults(parser, *funcs):
    ...
    in `flags` have to change from 'store_false' to 'store_true' and vice versa.
    ...
    for f in funcs:
        ...
        # TODO: see if in flag_config[cat]['kwargs'] 'action' and 'type' keys can be automatically set by default type
...
flag_config = dict(
    ...
    agent={
        'learning': {
            'names': ('-l', '--learn'),
            'kwargs': dict(action='store_true', help='forces the driving agent to use Q-learning', dest='learning')
        },
        'epsilon': {
            'names': ('-e', '--epsilon'),
    ...
    sim={
        ...
        'display': {'names': ('-d', '--no-display'),
                    'kwargs': dict(action='store_false', help='disable simulation GUI', dest='display')},
        'log_metrics': {
            'names': ('-L', '--log'),
